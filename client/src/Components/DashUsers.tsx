    ...
    <div>
      {currentUser.isAdmin && users.length > 0 ? (
        ...
        <div className="content">
          {
            users.map((user: any) => (
              <div key={user.username} className={styles.userLine}>
                <div>{new Date(user.createdAt).toLocaleDateString()}</div>
                <div className={styles.profileImgContainer}>
                    <img src={user.profileImg} alt={user.username} className={styles.profileImg} />
                </div>
                <div>{user.username}</div>
                <div>{user.email}</div>
                <div> <img src={user.isAdmin ? (tick) : (cross)} alt="d" /></div>
                <button 
                  onClick={() => {
                    setShowModal(true);
