    ...
    const uploadImage = async () => {
        ...
        if (imageFile) {
            ...
            uploadTask.on(
                ...
                (error) => {
                    setImageUploadError('Ошибка загрузки (Файл должен весить меньше 2МБ)')
  ...
  return (
    <div className={styles.dashProfile}>
        ...
        <form onSubmit={handleSubmit}>
            <input type="file" accept="image/*" onChange={handleImageChange} ref={pickFileRef} hidden />
            <div className={styles.userImgContainer} onClick={() => pickFileRef.current.click()} style={{position: 'relative'}}>
                {imageUploadProgress && (
                    <CircularProgressbar
                ...
                <img src={imageFileUrl || currentUser.profileImg || userDefaultImage} alt="userImg" className={imageUploadProgress < 100 ? 'opacity-50' : ''} />
            </div>
            {imageUploadError && <div><p>{imageUploadError}</p></div>}
            <input type="text" id="username" className={styles.inputUserInfo} placeholder="Имя пользователя" defaultValue={currentUser.username} onChange={handleChange} />

            <input type="email" id="email" className={styles.inputUserInfo} placeholder="Имя пользователя" defaultValue={currentUser.email} onChange={handleChange} />

            <div className={styles.passwordChanger}>
                <input type={`${showPassword ? 'text' : 'password'}`} id="password" className={styles.inputUserInfo} placeholder="Пароль" onChange={handleChange} />
                <div className={styles.showPassBtn} onClick={() => setShowPassword(!showPassword)}>
                    <img src={showPassword ? closedEye : openEye } alt="show" />
                </div>
            </div>

            <button type="submit" className={styles.saveInfo} disabled={loading || imageUploading}>{loading ? 'Загрузка...' : 'Сохранить'}</button>
            {
                currentUser.isAdmin && (
        ...
        {error && (
            <div className={styles.updateOutput}>{error}</div>
        )}
        {showModal && (
